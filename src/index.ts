...
async function start() {
    ...
    try {
        ...
        for (let item in result) {
            if (((result[item][`Tables_in_${config.database}`] as string).startsWith('_')) || exclude_table_names.includes((result[item][`Tables_in_${config.database}`] as string))) {
                continue
            }
...
async function create(table_name) {
    ...
    for (let i = 0; i < desc.length; i++) {
        ...
        if (desc[i].Default) {
            const regexp = /^[0-9]*$/
            if (!regexp.test(desc[i].Default)) { // 숫자
